...
def get_weather():
    ...
    days_temp =""
    #注意findALL find的区别 以及find_all
    for d in soup.findAll('ul', attrs={'class': 'days clearfix'}):
       days_temp += (d.getText())
    days_temp = days_temp.replace(' ','').replace('\r','').replace('\n','').replace('°','度').replace('/','到').replace('-','至').replace('明天',',明天').replace('后天',',后天')

    #print (days_temp)
    #提取出所有数字
    m = re.findall(r'([0-9]+)',days_temp)
    #print (m)
    #转换为对应的读数，注意map的用法
    m_read = list(map(numtozh,map(int,m)))
    #print (m_read)
    for i in range(0,len(m)):
        #将数字替换为对应的读法 每次只替换一个数字
        days_temp = days_temp.replace(m[i],m_read[i],1)
    #print (days_temp)
